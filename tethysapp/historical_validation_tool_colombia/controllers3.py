...
hydroviewer_figure = None
fixed_stats = None
hydroviewer_figure2 = None
...
def get_popup_response(request):
    ...
    global corrected_df
    global forecast_df
    global hydroviewer_figure
    global fixed_stats
    global hydroviewer_figure2
    ...
    try:
        ...
        hydroviewer_figure2 = geoglows.plots.forecast_stats(stats=fixed_stats, titles={'Station': nomEstacion + '-' + str(codEstacion), 'Reach ID': comid})
        print("hola2")
...
def get_time_series(request):
    ...
    try:
        ...
        try:
            ...
            # forecast_record = geoglows.streamflow.forecast_records(comid)
            # forecast_record[forecast_record < 0] = 0
            forecast_record1 = forecast_record.copy()
            forecast_record1 = forecast_record1.loc[forecast_record1.index >= pd.to_datetime(forecast_df.index[0] - dt.timedelta(days=8))]
            print("2")
            if len(forecast_record1.index) > 0:
                hydroviewer_figure.add_trace(go.Scatter(
                    name='1st days forecasts',
                    x=forecast_record1.index,
                    y=forecast_record1.iloc[:, 0].values,
                    line=dict(
                        color='#FFA15A',
                    )
                ))
            print("3")
            if 'x_vals' in locals():
                x_vals = (forecast_record1.index[0], forecast_df.index[len(forecast_df.index) - 1], forecast_df.index[len(forecast_df.index) - 1], forecast_record1.index[0])
                print("4")
            else:
                x_vals = (forecast_record1.index[0], forecast_df.index[len(forecast_df.index) - 1], forecast_df.index[len(forecast_df.index) - 1], forecast_record1.index[0])
                print("5")

            max_visible = max(forecast_record1.max(), max_visible)
    ...
    try:
        ...
        try:
            ...
            if len(fixed_records.index) > 0:
                hydroviewer_figure2.add_trace(go.Scatter(
                    name='1st days forecasts',
        ...
        if max_visible > r2:
            visible = True
            hydroviewer_figure2.for_each_trace(
                lambda trace: trace.update(visible=True) if trace.name == "Maximum & Minimum Flow" else (),
            )
        else:
            visible = 'legendonly'
        ...
        hydroviewer_figure2.add_trace(template('Return Periods', (r100 * 0.05, r100 * 0.05, r100 * 0.05, r100 * 0.05), 'rgba(0,0,0,0)', fill='none'))
        hydroviewer_figure2.add_trace(template(f'2 Year: {r2}', (r2, r2, r5, r5), colors['2 Year']))
        hydroviewer_figure2.add_trace(template(f'5 Year: {r5}', (r5, r5, r10, r10), colors['5 Year']))
        hydroviewer_figure2.add_trace(template(f'10 Year: {r10}', (r10, r10, r25, r25), colors['10 Year']))
        hydroviewer_figure2.add_trace(template(f'25 Year: {r25}', (r25, r25, r50, r50), colors['25 Year']))
        hydroviewer_figure2.add_trace(template(f'50 Year: {r50}', (r50, r50, r100, r100), colors['50 Year']))
        hydroviewer_figure2.add_trace(template(f'100 Year: {r100}', (r100, r100, r100 + r100 * 0.05, r100 + r100 * 0.05), colors['100 Year']))

        chart_obj = PlotlyView(hydroviewer_figure2)

        context = {
            'gizmo_object': chart_obj,
        }
        print("done time series bc")
